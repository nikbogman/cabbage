...
export interface IArrayResponseType<T> {
    error?: IFieldedError;
    data?: T[];
}

export function ArrayResponse<T>(classRef: Type<T>): Type<IResponseType<T>> {
    @ObjectType({ isAbstract: true })
    abstract class ResponseType implements IArrayResponseType<T> {
        @Field(() => FieldedErrorObjectType, { nullable: true })
        error?: IFieldedError;
