...
export class ItemService {
    ...
    async addItem(cartId: string, productId: string, productPrice: number, quantity: number) {
        const item = await this.prisma.item.findFirst({ where: { productId, cartId } });
    ...
    async removeItemBy(cartId: string, productId: string, productPrice: number, quantity: number) {
        const item = await this.prisma.item.findFirst({ where: { productId, cartId } });
